...
export class AuthService {
  ...
  public static async signup(payload: any, deviceFingerprint: string, clientDeviceId?: string): Promise<{ user: UserRecord; token: string }> {
    ...
    const newUser: UserRecord = {
      ...
      status: 'active',  // Grant active status with 7-day trial for new users
      subscription: {
        planId: isAdminEmail ? 'plan_1m' : 'plan_trial',
        planName: isAdminEmail ? '1 Month Plan' : '7-Day Free Trial',
        price: isAdminEmail ? 300 : 0,
        durationDays: isAdminEmail ? 30 : 7,
        activatedAt: new Date().toISOString(),
        expiresAt: new Date(Date.now() + (isAdminEmail ? 30 : 7) * 86400000).toISOString(),
        isActive: true,
        isExpired: false,
        deviceId
      },
